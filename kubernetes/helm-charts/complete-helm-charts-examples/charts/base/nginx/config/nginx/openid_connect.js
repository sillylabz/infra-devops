...
function oidcCodeExchange(r) {
  ...
  r.subrequest('/_token', 'code=' + r.variables.arg_code, function (reply) {
    if (reply.status == 504) {
    ...
    try {
      ...
      r.subrequest('/_id_token_validation', 'token=' + tokenset.id_token, function (reply) {
        if (reply.status != 204) {
        ...
        auth_token = tokenset[r.variables.oidc_token_type];
        r.variables.new_session = tokenset[r.variables.oidc_token_type];
...
//Below function is not used for broker configuration
