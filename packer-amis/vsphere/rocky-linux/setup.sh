...
# install packages
sudo dnf -y update && sudo dnf -y install open-vm-tools curl unzip perl perl-File-Temp python3
sudo dnf makecache --refresh && sudo dnf -y install cloud-init
...
echo "Installing cloud-init... and other packages"
sudo dnf -y install open-vm-tools curl unzip perl perl-File-Temp python3 cloud-init
